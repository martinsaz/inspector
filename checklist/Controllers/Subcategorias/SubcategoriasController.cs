...
namespace checklist.Controllers.Subcategorias
{
    public class SubcategoriasController : Controller
    {
        ...
        public IActionResult SubcategoriasABC()
        {
            return View();
        }
		...
		public async Task<ActionResult> Inicializa(string idEmpresa, string cadena, string empresa)
		{
            ...
            Opciones opc = await Utilerias.GetOpcion("01002002", idEmpresa, idRol, empresa, cadena);
		...
		public async Task<IActionResult> GetData(string idEmpresa, string cadena, string empresa)
        {
            ...
            string url = $"{Utilerias.UrlBase}ObtenerSubcategorias?idEmpresa={idEmpresa}&empresa={empresa}&cadena={cadena}";
            string idRol = Utilerias.IdRol;
            Opciones opc = await Utilerias.GetOpcion("01002002", idEmpresa, idRol, empresa, cadena);
            ...
            List<ListasPreguntasSubcategorias> respuesta = JsonConvert.DeserializeObject<List<ListasPreguntasSubcategorias>>(response.Content);
            List<ListasPreguntasSubcategorias> respuestaOrden = respuesta.OrderBy(r => r.Nombre).ToList();
        ...
        public async Task<IActionResult> GetSubcategoria(string lla, string idEmpresa, string cadena, string empresa)
        {
            
            string url = $"{Utilerias.UrlBase}ObtenerSubCategoria?idEmpresa={idEmpresa}&id={lla}&empresa={empresa}&cadena={cadena}";
            var client = new RestClient(url);
            var request = new RestRequest();
            request.Method = Method.Get;
            RestResponse response = await client.ExecuteAsync(request);
        ...
        public async Task<IActionResult> GuardaSubcategoria(string llav, string nomb, string idEmpresa, string cadena, string empresa, string nota = "")
        {
            ...
            if (string.IsNullOrEmpty(llav))
            {
                ...
                string url = $"{Utilerias.UrlBase}InsertarSubcategoria?empresa={empresa}&cadena={cadena}";
            ...
            else
            {
                ...
                var json = JsonConvert.SerializeObject(zona, Formatting.None, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
                string url = $"{Utilerias.UrlBase}ActualizarSubcategoria?id={llav}&empresa={empresa}&cadena={cadena}";
                var clientS = new RestClient(url);
                var request = new RestRequest();
