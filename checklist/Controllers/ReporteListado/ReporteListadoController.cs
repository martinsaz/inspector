...
using checklist.Models.ListaParaReporteListado;
using checklist.Models.Listas;
using checklist.Models.ReporteListado;
...
namespace checklist.Controllers.ReporteListado
{
    public class ReporteListadoController : Controller
    {
        ...
        public ReporteListadoController(IConfiguration configuration)
        {
        ...
        public async Task<ActionResult> Inicializa(string idEmpresa, string cadena, string empresa)
        {
            ...
            Opciones opc = await Utilerias.GetOpcion("03002000", idEmpresa, idRol, empresa, cadena);
        ...
        public async Task<IActionResult> GetPlantelesComboBox(string opci, string idEmpresa, string cadena, string empresa)
        {
            ...
            List<Lista> respuesta = JsonConvert.DeserializeObject<List<Lista>>(response.Content);
            string result = string.Empty;
            if (opci == "1") result = "<option value='0'> -- Todos --  </option> ";
            foreach (var resp in respuesta)
            {
                result += String.Format("<option value='{0}'>{1}</option> ", resp.id, resp.Nombre.Trim());
            }
        ...
        public async Task<IActionResult> GetListasCerradasComboBox(string opci, string idEmpresa, string cadena, string empresa)
        {
            ...
            List<Lista> respuesta = JsonConvert.DeserializeObject<List<Lista>>(response.Content);
            string result = string.Empty;
            result = "<option value='0'> -- Todas las listas --  </option> ";
            foreach (var resp in respuesta)
            {
                result += String.Format("<option value='{0}'>{1}</option> ", resp.id, resp.Nombre.Trim());
            }
        ...
        public async Task<IActionResult> GetData(string idLista, string idUsuario, string fechaInicia, string fechaFin, string idSucursal, string idEmpresa, string cadena, string empresa)
        {
            ...
            string url = string.Format("{0}api/Evaluaciones/Evaluacion/ObtenerListasReporte?fechaInicia={1}&fechaFin={2}&idSucursal={3}&idUsuario={4}&idLista={5}&empresa={6}&cadena={7}&idEmpresa={8}", Utilerias.UrlBase, fechaI, fechaF, idSucursal, idUsuario, idLista, empresa, cadena, idEmpresa);
            ...
            List<ListaParaReporteListado> respuesta = JsonConvert.DeserializeObject<List<ListaParaReporteListado>>(response.Content);
            ...
            foreach (var resp in respuesta)
            {
                ...
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode(resp.Fecha) + "\",");
                ...
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode("<button class='btn btn-primary' onclick='llenarComboProgramacionModal(\"" + resp.Evento + "\",\"" + resp.idSucursal + "\",\"" + resp.idLista + "\")'>Resultados</button>") + "\"");
        ...
        public async Task<IActionResult> GetDataListas(string idLista, string idUsuario, string idSucursal, string evento, string idEmpresa, string cadena, string empresa)
        {

            string url = string.Format("{0}GetReporteListado?idEmpresa={1}&empresa={2}&idLista={3}&evento={4}&idSucursal={5}&cadena={6}", Utilerias.UrlBase, idEmpresa, empresa, idLista, evento, idSucursal, cadena);
            Console.WriteLine(url);
            var client = new RestClient(url);
            var request = new RestRequest();
            request.Method = Method.Get;
            ...
            // Deserializar el JSON en una lista de objetos respcExperienciaVisible
            List<ReporteListados> respuesta = JsonConvert.DeserializeObject<List<ReporteListados>>(response.Content);
            ...
            foreach (var resp in respuesta)
            {
                ...
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode(resp.RespuestaValor) + "\",");
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode(resp.ValorCorrecto) + "\",");
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode(resp.FechaRespuesta) + "\",");
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode(resp.Notas) + "\",");
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode(resp.Explicacion) + "\",");
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode(resp.Usuario) + "\",");
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode(resp.TipoDeTarea) + "\",");
                sb.Append("\"" + HttpUtility.JavaScriptStringEncode(resp.Ponderacion) + "\"");
