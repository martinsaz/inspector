...
function Inicializa() {
    $.ajax({
        ...
        url: '/ReporteDinamico/Inicializa',
        data: {
...
function inicializarFiltroTipoPregunta () {
    const items = [
        { value: '0', text: 'Selecciona un tipo de pregunta' },
        { value: '1', text: 'Calificación' },
        { value: '5', text: 'Numeros' },
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#grData').DataTable({
                ...
                scrollCollapse: true,
                data: datos[0].DATA,
                columns: datos[0].COLUMNS,
                scrollX: true,
                createdRow: function (row, data, dataIndex) {
                    // Aplicar estilos a todas las columnas a partir del índice 4 en adelante
                    for (let colIndex = 3; colIndex < data.length; colIndex++) {
                        const cellValue = parseFloat(data[colIndex]);
                ...
                buttons: [
                    {
                        ...
                        action: function (e, dt, button, config) {
                            ...
                            const headers = datos[0].COLUMNS.map(col => col.title);
                            ws.addRow(headers);

                            // Añadir las filas de datos
                            datos[0].DATA.forEach((row, rowIndex) => {
...
function LlenaTablaRespuestaold() {
