        ...
        didOpen: function () {
            $.ajax({
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    ...
                    alert(`[GetData] status: ${jqXHR.status}, message: ${textStatus}`);
                }
            });
        }
    });
...
function Editar(cual) {
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            console.error(`[GetPuesto] status: ${xhr.status} responseText: ${xhr.responseText}`);
            alert(`[GetPuesto] status: ${xhr.status} responseText: ${xhr.responseText}`);
        }
    });
}
function Guardar() {
    const nombre = document.getElementById('txNombre').value.trim();

    // Validar si el nombre es "Administrador" o "Supervisor"
    if (nombre === "Administrador" || nombre === "Supervisor") {
        swal.fire({
            text: "El puesto con nombre 'Administrador' o 'Supervisor' ya existe y no puede ser duplicado.",
            icon: "error",
        ...
        didOpen: function () {
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    alert('[GuardaPuesto] status: ' + xhr.status + ' responseText: ' + xhr.responseText);
                }
            });
        }
    });
