...
function getSessionValue(key) {
    ...
    const context = document.getElementById("subcategorias-session-context");
    if (!context) {
        return "";
    }
...
function Editar(cual) {
    ...
    $.ajax({
        type: "GET",
        contentType: "application/json; charset=utf-8",
        url: '/Subcategorias/GetSubcategoria',
        data: {
            lla: cual,
            idEmpresa: getSessionValue('idEmpresa'),
            cadena: getSessionValue('cadenaBase64'),
        ...
        error: function (xhr, status, error) {
            console.error(`[GetSubcategoria] status: ${xhr.status} responseText: ${xhr.responseText}`);
            alert(`[GetSubcategoria] status: ${xhr.status} responseText: ${xhr.responseText}`);
        }
    });
}
    ...
    swal.fire({
        ...
        didOpen: function () {
            $.ajax({
                ...
                type: "GET",
                contentType: "application/json; charset=utf-8",
                url: '/Subcategorias/GuardaSubcategoria',
                data: {
                ...
                error: function (xhr, status, error) {
                    ...
                    console.error('[GuardaSubcategoria] status:', xhr.status, 'responseText:', xhr.responseText);
                }
            });
        }
    });
